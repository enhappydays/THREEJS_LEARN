...
controls.enableDamping = true; //阻尼效果开启
// 初始化loader
...
// 添加点光源
const postLight=new THREE.PointLight(0xffffff,50)
postLight.position.set(0.1,2.4,0);
...
function render() {
  requestAnimationFrame(render); //动画帧
...
let scenes = [
  ...
  },
  {
    ...
    callback: () => {
      // 执行函数切
      translateCamera(new THREE.Vector3(20, 0, 43), new THREE.Vector3(0, 0, 0));
    },
  },
  {
    text: "愿与你探寻世界的每一个角落",
...
<template>
    ...
    >
      <div v-for="item in scenes" style="width: 100vw; height: 100vh" :key="item">
        <h1 style="padding: 50px 50px; font-size: 32px; color: #fff">
          {{ item.text }}
        </h1>
      </div>
    </div>
